...
function StatsCard({ data }) {
  ...
  return (
    <div className="w-40 h-40 flex justify-center items-center  relative overflow-hidden">
      <ImageUI text={data.id} source={first.url} dimentions={dimentions} />
      <div className="absolute w-full h-full bottom-0 bg-gradient-to-t from-[--primary-dark] to-transparent flex flex-col justify-end items-start p-2">
        <span className="text-white text-sm w-full flex gap-2">
          <article className="text-[13px]">{data.id}</article>
        </span>
      </div>
    </div>
  );
